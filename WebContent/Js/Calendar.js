...
Calendar.prototype = {
  ...
  LangData:{
   months:
    ...
    _close:
    {
      english:"Close",
      francais:"Fermer",
      deutsch:"Fenster schlie&szlig;en",
      espanol:"Cerrar"
    }
  ...
  build:function(theMonths, theWeekDays)
  {
    ...
    var f = first.getDay();
    var how_many = ((num_days[get_month] - 1) + f);
      ...
      else
      {
        if(this.startdate && this.enddate)
        {
          ...
          var end = new Date();
			...
			/* ADD TO DATE */
			var plusMonth = (end_month - now.getMonth());
			var plusDate = (end_date - now.getDate());
			var plusYear = (end_year - now.getFullYear());

			/* SUBTRACT FROM DATE */
			var minusMonth = (now.getMonth() - start_month);
			var minusDate = (now.getDate() - start_date);
			var minusYear = (now.getFullYear() - start_year);

			/* CONVERT START DATE TO DATE FORMAT */
			start.setMonth(now.getMonth() - minusMonth);
			start.setDate(now.getDate() - minusDate);
			start.setYear(now.getFullYear() - minusYear);

			/* CONVERT END DATE TO DATE FORMAT */
			end.setMonth(now.getMonth() + plusMonth);
			end.setDate(now.getDate() + plusDate);
			end.setYear(now.getFullYear() + plusYear);
        }
        ...
        if(this.datepicker && withinBounds)
        {
          ...
          a.rel = ((get_month + 1) + "/" + (i - f + 1) + "/" + get_year);
          td.appendChild(a);
        }
        else
        {
    ...
    if(this.popup)
    {
      ...
      a.onclick = function()
      {
        ...
        if(iframe) document.getElementById("ie_mask").parentNode.removeChild(iframe);
    ...
    if(this.datepicker)
    {
      this.date_picker(links, theMonths, theWeekDays);
...
addEvent(window, "load", function()
{
  ...
  if(document.getElementById('calendarStartDate')){
    var calendarStartDate = new Calendar({
      id:"calendarStartDate",
      format:"short",
      datepicker:"startDate",
      popup:"calendarIconStart",
      startdate:(m + "/" + d + "/" + y),
      enddate:(m + "/" + d + "/" + (y + 1))
    });
  }
